...
import edu.dhbw.mannheim.tigers.sumatra.view.visualizer.internals.field.FieldPanel.ELimitField;
...
public class SSLVisionCamDetectionTranslator
{
			...
			long receivedTimeStamp, long packetCount, TeamProps teamProps)
	{
		...
		final VisualizerPanel panel = new VisualizerPanel();
		...
		{
			if (!((panel.getFieldPanel().getLimitField() == ELimitField.LEFTSIDE && bot.getX() > 0) ||
					(panel.getFieldPanel().getLimitField() == ELimitField.RIGHTSIDE && bot.getX() < 0)))
			{
		...
		for (final SSL_DetectionRobot bot : detectionFrame.getRobotsYellowList())
		{
			if (!((panel.getFieldPanel().getLimitField() == ELimitField.LEFTSIDE && bot.getX() > 0) ||
					(panel.getFieldPanel().getLimitField() == ELimitField.RIGHTSIDE && bot.getX() < 0)))
			{
				yellows.add(convertRobot(bot, haveToTurn));
			}
		}
		...
		{
			if (!((panel.getFieldPanel().getLimitField() == ELimitField.LEFTSIDE && ball.getX() > 0) ||
					(panel.getFieldPanel().getLimitField() == ELimitField.RIGHTSIDE && ball.getX() < 0)))
			{
