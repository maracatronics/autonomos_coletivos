...
public class GrSimBot extends ABot implements IWorldFrameConsumer
{
	...
	private List<Float>					errXMemory = new ArrayList<Float>();
	private List<Float>					errYMemory = new ArrayList<Float>();
	private List<Float>					errZMemory = new ArrayList<Float>();
	...
	private static final int		GRAU_PID	= 3;
	...
	private void handleMove(final TigerMotorMoveV2 move, final GrSimConnection con)
	{
		...
		if (move != null)
		{
			// CONTROLE CORPO RIGIDO
			/*
			TrackedTigerBot bot = latestWorldFrame.getBot(getBotID());
			System.out.println("BEFORE IF");
			if (bot != null) {
				float tarVelX = move.getY();
				float tarVelY = -move.getX();
				float tarVelZ = move.getW();
				
				float curVelX = bot.getVel().x();
				float curVelY = bot.getVel().y();
				float curVelZ = bot.getaVel();
				
				float errVelX = tarVelX - curVelX;
				float errVelY = tarVelY - curVelY;
				float errVelZ = tarVelZ - curVelZ;
				
				float outVelX = curVelX ;
				
				errXMemory.add(2, errXMemory.get(1));
				errXMemory.add(1, errXMemory.get(0));
				errXMemory.add(0, errVelX);
				
				errYMemory.add(2, errYMemory.get(1));
				errYMemory.add(1, errYMemory.get(0));
				errYMemory.add(0, errVelY);
				
				errZMemory.add(2, errZMemory.get(1));
				errZMemory.add(1, errZMemory.get(0));
				errZMemory.add(0, errVelZ);
				
				System.out.println("err[k]: " + errXMemory.get(0));
				System.out.println("HERE: " + botId.getNumber());
			}*/
	...
	private void handlePositionMove(final TigerSkillPositioningCommand cmd, final GrSimConnection con)
	{
		...
		{
			TrackedTigerBot bot = latestWorldFrame.getBot(getBotID());
			System.out.println("GrSim Bot:" + getBotID());
			if (bot == null)
			{
